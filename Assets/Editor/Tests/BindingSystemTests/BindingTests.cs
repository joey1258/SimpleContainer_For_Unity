...
namespace uMVVMCS_NUitTests
{
    ...
    public class BindingTests
    {
        ...
        /// <summary>
        /// 测试 ToAddress 方法是否使 ADDRESS 类型的无 id binding 的值约束转换为 SINGLE 类型
        ...
        public void ToSelf_NullId_ConstraintChangeTOSingle()
        {
            ...
            //Act
            IBinding binding = binder.Bind<object>().ToAddress();
            //Assert
            Assert.AreEqual(ConstraintType.SINGLE, binding.constraint);
        }

        /// <summary>
        /// 测试 ToAddress 方法是否使 ADDRESS 类型的无 id binding 是否被 binder 保存
        ...
        public void ToSelf_NullId_BinderSaveCorrect()
        {
            ...
            IBinder binder = new Binder();
            //Act
            binder.Bind<object>().ToAddress();
            //Assert
            Assert.AreEqual(1, binder.GetAll().Count);
        }

        /// <summary>
        /// 测试 ToAddress 方法是否使 ADDRESS 类型的有 id binding 的值约束转换为 SINGLE 类型
        /// 由于绑定自身的type作为自己的值的缘故，自身的值只有一个，所以值的类型也没有必要保留为 MULTIPLE
        /// </summary>
        [Test]
        ...
        /// <summary>
        /// 测试 ToAddress 方法是否使 ADDRESS 类型的有 id binding 是否被 binder 保存
        /// </summary>
        [Test]
        public void ToSelf_HasId_BinderSaveCorrect()
        {
            ...
            IBinder binder = new Binder();
            //Act
            binder.Bind<object>().ToAddress().As(1);
            //Assert
            Assert.AreEqual(
                true,
        ...
        /// <summary>
        /// 测试 To（instance） ADDRESS 类型 binding,binder 是否正确保存
        /// </summary>
        [Test]
        ...
        /// <summary>
        /// 测试 To（instance） ADDRESS 类型 binding,bindingType 是否正确转换为 Singleton
        /// </summary>
        [Test]
        ...
        /// <summary>
        /// 测试 To（instance） ADDRESS 类型 binding,值约束是否正确转换为 Single
        /// </summary>
        [Test]
        ...
        /// <summary>
        /// 测试 ReBind ADDRESS 类型 binding,binder 是否正确保存
        /// </summary>
        [Test]
        ...
        public void ChainToSelf_TempBindingToType_BinderSaveAndValueOverWhiteCorrect()
        {
            ...
            IBinder binder = new Binder();
            //Act
            IBinding binding = binder.Bind<object>().ToAddress().To(typeof(object));
            //Assert
            Assert.AreEqual(
                true,
                binder.GetAll().Count == 1 &&
        ...
        public void ChainToSelf_TempBindingToInstance_BinderSaveAndValueOverWhiteCorrect()
        {
            ...
            object o = new object();
            //Act
            IBinding binding = binder.Bind<object>().ToAddress().To(o);
            //Assert
            Assert.AreEqual(
                true, 
                binder.GetAll().Count == 1 &&
        ...
        /// <summary>
        /// 测试链式添加值给 ADDRESS 类型 binding,binder 是否正确保存以及后保存的值是否覆盖之前的值
        /// 由于保存实例时已经改变了 binding 的类型，而后保存的 ToAddress 只会改变约束类型，所以 binder 还是会进行保存（同时还由于第一保存实例时 binder 已经进行过保存了，而 ToAddress 并不会调用 UnBind 方法，所以即使转换为 ADDRESS 类型也一样不会从 binder 中移除
        /// </summary>
        [Test]
        public void ChainTo_TempBindingToSelf_BinderSaveAndValueOverWhiteCorrect()
        {
            ...
            object o = new object();
            //Act
            IBinding binding = binder.Bind<object>().To(o).ToAddress();
            //Assert
            Assert.AreEqual(
                true,
                binder.GetAll().Count == 1 &&
        ...
        /// <summary>
        /// 测试链式添加值给 ADDRESS 类型 binding,值的数量是否正确
        /// </summary>
        [Test]
